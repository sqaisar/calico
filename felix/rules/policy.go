...
func (r *DefaultRuleRenderer) ProtoRulesToIptablesRules(protoRules []*proto.Rule, ipVersion uint8, chainComments ...string) []Rule {
	...
	for len(rules) > 0 {
		if _, ok := rules[len(rules)-1].Action.(ReturnAction); ok {
...
func (r *DefaultRuleRenderer) ProtoRuleToIptablesRules(pRule *proto.Rule, ipVersion uint8) []Rule {
	...
	if markBit != 0 {
		...
		rs = append(rs, Rule{
			Match:  match,
			Action: SetMarkAction{Mark: markBit},
		})
		match = Match().MarkSingleBitSet(markBit)
	}
...
func (r *matchBlockBuilder) AppendPortMatchBlock(
	...
	for _, split := range numericPortSplits {
		m := appendProtocolMatch(Match(), protocol, logCxt)
		m = srcOrDst.AppendMatchPorts(m, split)
		r.Rules = append(r.Rules, Rule{
			Match:  m,
			Action: SetMarkAction{Mark: markToSet},
		})
	}
...
func (r *matchBlockBuilder) AppendNegatedCIDRMatchBlock(cidrs []string, srcOrDst srcOrDst) {
	...
	for _, cidr := range cidrs {
		r.Rules = append(r.Rules,
			Rule{
				Match:  srcOrDst.MatchNet(cidr),
				Action: ClearMarkAction{Mark: r.markAllBlocksPass},
			},
		)
	}
}
...
func (r *matchBlockBuilder) finishPositiveBlock() {
	...
	//
	r.Rules = append(r.Rules, Rule{
		Match:  Match().MarkClear(r.markThisBlockPass),
		Action: ClearMarkAction{Mark: r.markAllBlocksPass},
	})
}
...
func (sod srcOrDst) MatchNet(cidr string) MatchCriteria {
	switch sod {
	case src:
		return Match().SourceNet(cidr)
	case dst:
		return Match().DestNet(cidr)
	}
	log.WithField("srcOrDst", sod).Panic("Unknown source or dest type.")
	return nil
}

func (sod srcOrDst) AppendMatchPorts(m MatchCriteria, pr []*proto.PortRange) MatchCriteria {
	switch sod {
	case src:
...
func (sod srcOrDst) MatchIPPortIPSet(setID string) MatchCriteria {
	switch sod {
	case src:
		return Match().SourceIPPortSet(setID)
	case dst:
		return Match().DestIPPortSet(setID)
	}
	log.WithField("srcOrDst", sod).Panic("Unknown source or dest type.")
	return nil
}
...
func appendProtocolMatch(match MatchCriteria, protocol *proto.Protocol, logCxt *log.Entry) MatchCriteria {
	if protocol == nil {
		return match
	}
...
func (r *DefaultRuleRenderer) CalculateRuleMatch(pRule *proto.Rule, ipVersion uint8) MatchCriteria {
	match := Match()
